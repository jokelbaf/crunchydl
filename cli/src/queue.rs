...
#[cfg(test)]
mod tests {
    use super::{Queue, QueueSelection, QueueState};

    #[test]
    fn queue_round_trips_and_recovers_running_jobs() {
        let root = std::env::temp_dir().join(format!("crunchydl-queue-{}", uuid::Uuid::new_v4()));
        let path = root.join("queue.json");
        let mut queue = Queue::load(&path).expect("create queue");
        let ids = queue
            .add(
                [crunchydl::MediaTarget::Episode("episode-id".to_string())],
                QueueSelection::default(),
            )
            .expect("add item");
        queue.mark_running(ids[0]).expect("mark running");

        let queue = Queue::load(&path).expect("reload queue");
        assert_eq!(queue.items()[0].state, QueueState::Pending);
        assert_eq!(queue.items()[0].attempts, 1);
        let _ = std::fs::remove_dir_all(root);
    }

    #[test]
    fn failed_queue_item_retains_the_actionable_error() {
        let root = std::env::temp_dir().join(format!("crunchydl-queue-{}", uuid::Uuid::new_v4()));
        let path = root.join("queue.json");
        let mut queue = Queue::load(&path).expect("create queue");
        let ids = queue
            ...
            .expect("add item");
        queue
            .mark_failed(ids[0], "Crunchyroll API error: request failed (HTTP 429)")
            .expect("mark failed");

        let queue = Queue::load(&path).expect("reload queue");
        assert_eq!(
            queue.items()[0].failure.as_deref(),
            Some("Crunchyroll API error: request failed (HTTP 429)")
        );
        let _ = std::fs::remove_dir_all(root);
    }

    #[test]
    fn individual_items_can_be_retried_and_removed() {
        let root = std::env::temp_dir().join(format!("crunchydl-queue-{}", uuid::Uuid::new_v4()));
        let path = root.join("queue.json");
        let mut queue = Queue::load(&path).expect("create queue");
        let ids = queue
            .add(
                [
                    crunchydl::MediaTarget::Episode("failed".to_string()),
                    crunchydl::MediaTarget::Episode("pending".to_string()),
                ],
                QueueSelection::default(),
            )
            .expect("add items");
        queue.mark_failed(ids[0], "network failed").expect("fail");
        assert!(queue.retry(ids[0]).expect("retry"));
        assert_eq!(queue.items()[0].state, QueueState::Pending);
        assert!(queue.remove(ids[1]).expect("remove"));
