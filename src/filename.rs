...
mod tests {
    ...
    fn sanitizes_windows_names_and_traversal() {
        assert_eq!(sanitize_component("CON", 100), "_CON");
        assert_eq!(sanitize_component("../bad:name. ", 100), ".._bad_name");
        assert!(output_path(Path::new("out"), Path::new("../bad.mkv")).is_err());
    }

    #[test]
    fn hierarchical_layout_sanitizes_every_component() {
        let episode: crunchyroll_rs::Episode = serde_json::from_value(
            serde_json::from_str::<serde_json::Value>(include_str!(
                "../tests/fixtures/api/season_episodes.json"
            ))
            .unwrap()[0]
                .clone(),
        )
        .unwrap();
        let media = ResolvedMedia::from_episode(&episode);
        let layout = OutputLayoutTemplate::compile(
            "{series}/Season {season_number}/{episode} - {title} [{media_id}]",
        )
        .unwrap();
        assert_eq!(
            layout.render(&media, Some(1080), &[], 180),
            PathBuf::from("My Series/Season 0/1 - Episode One [EP1].mkv")
        );
        assert!(OutputLayoutTemplate::compile("../{title}").is_err());
    }
}
